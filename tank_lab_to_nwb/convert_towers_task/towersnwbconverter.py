...
class TowersNWBConverter(NWBConverter):
    ...
    def __init__(self, source_data, ttl_source: PathType):
        ...
        assert len(rising_times) > 0, (
            f"No TTL events found in ttl_source file ({ttl_source})."
        )
        start_time = recording.frame_to_time(rising_times[0])

        for interface_name in ["SpikeGLXRecording", "SpikeGLXLFP"]:
            if (
                interface_name in self.data_interface_objects
            ):  # specified in source_data
                interface_extractor = self.data_interface_objects[
                    interface_name
                ].recording_extractor
    ...
    def get_metadata(self):
        vermin_file_path = Path(
            self.data_interface_objects["VirmenData"].source_data["file_path"]
        )
        session_id = vermin_file_path.stem

        metadata = super().get_metadata()
        metadata["NWBFile"].update(
            session_id=session_id, institution="Princeton", lab="Tank"
        )
